...
def spurning(id):
    conn = sqlite3.connect('/home/pallih/x13/db/db.sqlite')
    cur = conn.cursor()
    #cur.execute('SELECT * from question_responses WHERE q_id = '+str(id)+' ORDER BY candidate_name')
    #rows = cur.fetchall()
    ...
    #cur.execute('SELECT * from candidates WHERE candidate_answer_status LIKE "cand ok"')
